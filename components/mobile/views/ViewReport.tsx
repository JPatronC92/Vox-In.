...
export const ViewReport: React.FC<Props> = ({
    t, status, isTranscribing, preTranscription, handleRunAnalysis, report, lang,
    seekTo, localAnalysis, duration, mimeType, fileHash, logs, audioUrl
}) => {
        ...
        <div className="space-y-6 pb-24">
                ...
                <div className="flex-1">
                    {isTranscribing ? (
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className="text-xs text-slate-300 truncate max-w-[150px]">{preTranscription || "Has not run"}</p>
                        </div>
                    )}
                </div>
                <div className="w-32">
            ...
            {report ? (
                <div className="space-y-6 animate-in slide-in-from-bottom duration-500">
                    {/* Verdict Card */}
                    <div className={`p-6 rounded-3xl border ${report.anomalyDetection.riskLevel === 'ALTO' || report.anomalyDetection.riskLevel === 'HIGH' ? 'border-rose-500/30 bg-rose-500/5' : 'border-brand-500/30 bg-brand-500/5'} shadow-xl`}>
                        <div className="flex justify-between items-start mb-4">
                            <div>
                                <span className="text-[10px] font-black text-brand-500 uppercase tracking-[0.2em]">{t.resultReport}</span>
                                <h2 className="text-2xl font-extrabold text-white mt-2 leading-none">{t.resultVerdict} <br /><span className={report.anomalyDetection.riskLevel === 'ALTO' || report.anomalyDetection.riskLevel === 'HIGH' ? 'text-rose-500' : 'text-brand-500'}>{report.anomalyDetection.riskLevel}</span></h2>
                            </div>
                            <div className="text-right">
                                <p className={`text-4xl font-mono font-black ${report.anomalyDetection.riskLevel === 'ALTO' || report.anomalyDetection.riskLevel === 'HIGH' ? 'text-rose-500' : 'text-brand-500'}`}>{report.anomalyDetection.overallScore}</p>
                            </div>
                        </div>
