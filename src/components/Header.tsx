...
const Header: React.FC<HeaderProps> = ({ status, isTranscribing }) => {
  const { lang, setLang, t } = useLanguage();
  const { isTauri, handleResetApiKey } = useAuth();
    ...
    <header className="px-4 py-3 nav-blur border-b border-white/5 flex items-center justify-between z-50 shrink-0">
      <div className="flex items-center gap-3">
        ...
        <div className="overflow-hidden">
          <h1 className="text-base font-extrabold tracking-tight text-white leading-none whitespace-nowrap truncate">
            VOX INTELLIGENCE
          </h1>
      ...
      <div className="flex items-center gap-2 md:gap-4 shrink-0">
        ...
        <div className="flex bg-white/5 p-1 rounded-lg border border-white/5 shrink-0">
          <button
            onClick={() => setLang('es')}
            className={`px-2 py-1 text-xs font-black rounded transition-all ${lang === 'es' ? 'bg-brand-500 text-deep-950' : 'text-slate-500'}`}
          >
            ES
          </button>
          <button
            onClick={() => setLang('en')}
            className={`px-2 py-1 text-xs font-black rounded transition-all ${lang === 'en' ? 'bg-brand-500 text-deep-950' : 'text-slate-500'}`}
          >
            EN
          </button>
        </div>
        ...
        <div className="px-2 py-1 rounded-full border border-white/5 bg-white/5 flex items-center gap-2 shrink-0">
          <div
            className={`w-1.5 h-1.5 rounded-full ${status === 'ANALIZANDO' || isTranscribing ? 'bg-amber-400 animate-pulse' : 'bg-brand-500'}`}
          ></div>
          <span className="text-sm font-bold text-white uppercase hidden sm:block">
            {isTranscribing ? (lang === 'es' ? 'PROCESANDO' : 'PROCESSING') : status}
          </span>
        </div>
