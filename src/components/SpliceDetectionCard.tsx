...
const SpliceDetectionCard: React.FC<Props> = ({ cutsDetected, timestamps, lang, onSeek }) => {
  ...
  return (
    <section
      className={`enterprise-panel rounded-2xl overflow-hidden border ${hasEdits ? 'border-amber-500/30' : 'border-brand-500/30'}`}
    >
      <div className="p-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center ${hasEdits ? 'bg-amber-500/10' : 'bg-brand-500/10'}`}
          >
