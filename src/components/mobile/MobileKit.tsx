...
export const MobileBottomNav: React.FC = () => {
  ...
  const getActiveTab = (path: string) => {
    ...
    if (path.includes('report')) return 'RESULTADOS';
  ...
  const tabs = [
    { id: 'AUDIO', path: '/capture', icon: Layers, label: lang === 'es' ? 'Captura' : 'Capture' },
    { id: 'VISUAL', path: '/visual', icon: Activity, label: lang === 'es' ? 'Visual' : 'Visual' },
    {
      id: 'RESULTADOS',
      path: '/report',
      icon: FileSearch,
      label: lang === 'es' ? 'Reporte' : 'Report',
    },
  ];
      ...
      <div className="flex justify-around items-center h-full px-2">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              <div
                className={`p-1.5 rounded-xl transition-colors ${isActive ? 'text-brand-400 bg-brand-500/10' : 'text-slate-500'}`}
              >
                <Icon size={24} strokeWidth={isActive ? 2.5 : 2} />
              </div>
              <span
                className={`text-[10px] font-bold uppercase tracking-wider ${isActive ? 'text-brand-400' : 'text-slate-500'}`}
              >
                {tab.label}
              </span>
            </button>
...
export const MobileLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  return (
    <div className="flex flex-col min-h-screen bg-deep-950 text-slate-100 font-sans selection:bg-brand-500/30 pb-24">
      ...
      <main className="flex-1 relative z-10 px-4 py-6 content-safe-area">{children}</main>
    </div>
  );
};
...
export const MobileCard: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <div
    className={`bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-2xl p-5 shadow-lg ${className}`}
  >
    {children}
  </div>
);
...
> = ({ children, variant = 'primary', className = '', ...props }) => {
  const variants = {
    primary:
      'bg-gradient-to-br from-brand-500 to-brand-600 text-deep-950 shadow-[0_4px_20px_-4px_rgba(16,185,129,0.4)]',
