...
export const ViewCapture: React.FC<Props> = ({
  ...
  const { fileHash, audioUrl, mimeType, duration, isPlaying, setIsPlaying, audioRef } = useAudio();
      ...
      <section>
        ...
        <MobileCard className="space-y-6">
          <div className="flex flex-col gap-4">
            ...
            >
              <div
                className={`w-24 h-24 rounded-full flex items-center justify-center transition-all ${isRecording ? 'bg-rose-500 text-white animate-pulse shadow-[0_0_50px_rgba(244,63,94,0.4)]' : 'bg-deep-950 text-brand-400 border border-brand-500/20'}`}
              >
          ...
          <FileUploader isTauri={isTauri} onFileSelected={processAudioSource} label={t.upload} />
        </MobileCard>
      </section>
        ...
        <MobileCard>
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-mono text-slate-500 truncate uppercase mb-1">
                {mimeType}
              </p>
