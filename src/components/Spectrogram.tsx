...
const Spectrogram: React.FC<Props> = ({ active, editPoints, duration = 0, lang = 'es' }) => {
  ...
  const t = {
    es: {
      freq: 'Frecuencia',
      time: 'Tiempo',
      hint: 'Usa scroll para Zoom, arrastra para Desplazar',
    },
  ...
  return (
    <div className="w-full h-full bg-slate-950 relative overflow-hidden group cursor-crosshair select-none">
      ...
      {hoverData && (
        <>
          <div
            className="absolute top-0 bottom-0 border-l border-brand-500/20 pointer-events-none"
            style={{ left: hoverData.x }}
          ></div>
          <div
            className="absolute left-0 right-0 border-t border-brand-500/20 pointer-events-none"
